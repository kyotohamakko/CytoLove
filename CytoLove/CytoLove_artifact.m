...
[src_tem, ei, orient, AODF_F] = res_trim2(src_tem, ei, orient, AODF_F, [40, 100], [40,100]);
...
for i=1:1:iter
    ...
    switch pick
       ...
       case 'connect'
          connect(pg, elmax, bta1, bta2, T_conn, b, T, E_total);
       case 'move_rotate'
...
legend('E\{X\}(M)','E_{int}\{X\}(M)','E_{ext}\{X\}(M)','number of particles', 'location', 'east');
...
function connect(pg, elmax, bta1, bta2, T_conn, b, T, E_total)
    if pg.isempty()
        return;
    end
...
%     e_new_id = randsample(1:1:size(e_candidate, 2),1,true,p_e);
    ...
    [R, E0, dE] = conn_R(p1, e0, e0_idx, e_new, e_new_id, p_e, 8.0, bta2, T);
...
%     [ori_new, p_new_ori] = ori_noise(p.orient, (T/p.scale) + 1.0);
...
function track_death(pg, dl, bta1, bta2, cw1, cw2, k, c, T, E_total)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     n_p = [cos((p.orient/180)*pi), sin((p.orient/180)*pi)];
%     n_padj = [cos((p_adj.orient/180)*pi), sin((p_adj.orient/180)*pi)];
%     p_n = G_n(n_p, n_padj, (T/p.scale)+1.0);
    p_n = 1.0;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     pos_new = floor(pos+random('Normal',0,sig,1,2));
    i = 0;
...
%         E0 = C_Edge(p, e0.beg, e0.point, e0.ter, bta1, bta2);
...
%     E_new = bta1*(sqrt(dot(p.pos-e_new.point.pos, p.pos-e_new.point.pos)) - (p.scale+e_new.point.scale)/4.0)^2;
